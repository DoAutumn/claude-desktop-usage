...
> 一个小巧的 macOS 桌面浮窗，一眼查看你的 Claude.ai 用量 —— 不用打开浏览器。
...
**与 Anthropic 无关。** 本工具是个人小工具，借用你本地已存的 Claude 桌面 app 会话，访问 claude.ai 内部（非公开、无文档）的用量端点。Anthropic 不为其背书，该端点可能随时变动或失效。
